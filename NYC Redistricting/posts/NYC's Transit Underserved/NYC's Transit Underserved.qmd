...
bus<-import('~/data/mta/MTA_Bus_Speeds__Beginning_2015.csv')
bus<-bus%>%
  clean_names()
...
c_map<-read_sf('~/data/NYCShapefile/nyct2020_22b')
...
bus_map<-read_sf('~/data/BusShapefile')
bm_sf <- st_transform(bus_map, "+init=epsg:4326")

prelim<-read_sf('~/data/PrelimOneShapefile/shapefile')
prelim_sf <- st_transform(prelim, "+init=epsg:4326")

cc_map<-read_sf('~/data/CCShapefile/nycc_22b')
...
subway_stops<-read_sf('~/data/subwaystop')
sub <- st_transform(subway_stops, "+init=epsg:4326")
```

```{r}
#| include: false
mobility <- read_csv("~/data/mta/Citywide_Mobility_Survey_-_Trip_Survey_2019.csv")
nta<-sf::read_sf('~/data/NTAShapefile')
cdt<-read_sf('~/data/CDT')

survey<-read_sf('~/data/survey_zones')
```
